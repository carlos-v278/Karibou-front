...
<template>
  <div class="advertisement-view">
      ...
      >
        ...
        >
          ...
          <div class="priview">
            <img
              v-if="
                props.advertisement?.pictures?.length > 0 &&
                index < props.advertisement?.pictures?.length
              "
              :src="[
                index + 1 < props.advertisement?.pictures?.length
                  ? props.advertisement?.pictures[index + 1]?.file
                  : props.advertisement?.pictures[0]?.file,
              ]"
              alt="image-annonce"
            />
          </div>
        </swiper-slide>
        <swiper-slide v-if="props.advertisement?.pictures?.length === 0">
          <img
    ...
    <div class="advertisement-view-main">
      <div class="top">
        <span class="type">{{
          advertisement.type.charAt(0).toUpperCase() +
          advertisement.type.slice(1)
        }}</span>
      </div>
      <div class="middle">
        ...
        <span
          class="price"
          v-if="
            advertisement?.type === 'vente' || advertisement?.type === 'job'
          "
        >
          {{ advertisement.price }} €
        </span>
      ...
      <div class="bottom">
        <div class="description" v-html="advertisement?.description"></div>

        <div class="owner">
          <ListItem :border="false">
            <template #icons>
              <div class="icon row items-center justify-center">
                <my-avatar :picture="advertisement?.owner?.picture"></my-avatar>
              </div>
            </template>
            <template #infos>
              <span
              ...
              <span
                v-if="
                  advertisement?.owner?.roles.includes('ROLE_TENANT_CREATE')
                "
                style="font-size: 13px; font-weight: 300"
                class="items-baseline"
              >

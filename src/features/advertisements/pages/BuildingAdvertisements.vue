...
const buildingAdvertisements = ref<Advertisement[]>();
...
async function loadBuildingDetails() {
  ...
  await buildingService
    .getBuildingDetails(route.params.id)
...
<template>
  <q-page class="advertisements row justify-center">
    <div class="left" :class="{ mobile_form: displayForm }">
      ...
      <div class="buildings-advertisement">
        <PropertyDetails
          ...
          component-title="neighbourAdvertising"
          type="severalAdvertisingType"
        >
          <template #actions>
            <q-icon
    ...
    <div class="right">
      ...
      <view-advertisement
        ...
        v-if="
          componentToDisplay === 'advertisement' &&
          Object.keys(currentAdvertisement).length != 0
        "
      >
      </view-advertisement>
    </div>
